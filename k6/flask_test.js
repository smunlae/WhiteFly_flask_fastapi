...
export default function () {
  ...
  check(homeRes, {
    "flask home status is 200": (r) => r.status === 200,
  });
  ...
  check(syncRes, {
    "flask sync returns redirect": (r) => r.status === 302 || r.status === 303,
  });
  ...
  check(asyncRes, {
    "flask async returns redirect": (r) => r.status === 302 || r.status === 303,
  });
